...
public class TOY_PeekAt { 
    ...
    public TOY_PeekAt(String filename, int pc) {
        ...
        ****************************************************************/
        ...
        public TOY_PeekAt(String filename, int pc, int watchAddress, int watchRegister, boolean isVerbose) {
        this.pc = pc;
...
if (pc == watchAddress && watchRegister >= 0 && watchRegister < 16) {
    StdOut.printf("PC=%02X  R[%X]=%04X\n", pc, watchRegister, reg[watchRegister]);
}
            // stdout
  ...
  public static void main(String[] args) { 
        ...
        // optional watch arguments
        int watchAddress = -1;
        int watchRegister = -1;
        if (args.length - argIndex >= 2) {
            watchAddress = Integer.parseInt(args[argIndex++], 16);
            watchRegister = Integer.parseInt(args[argIndex++], 16);
        }

        TOY_PeekAt toy = new TOY_PeekAt(filename, pcStart, watchAddress, watchRegister, isVerbose);
        toy.run();
    }
}