...
public class TOY {
        ...
        while (true) {
            ...
            switch (op) {
                case 1:
                    ...
                    break; // add
                case 2: // multiply
                    int product = (short) reg[s] * (short) reg[t];
                    reg[d] = (product >>> 16) & 0xFFFF;
                    int next = (d + 1) % 16;
                    if (next != 0)
                        reg[(d + 1) % 16] = product & 0xFFFF;
    ...
    public static void main(String[] args) {
        ...
        if (!isVerbose && args.length > 0)
            filename = args[0];
        if (isVerbose && args.length > 1)
            filename = args[1];
