...
public class Tabulate {
    ...
    public static void PrintRow(  String[] row, int[] columnWidth, String separator){
        ...
        String paddedSep = " ".repeat(SEP_WIDTH) + separator + " ".repeat(SEP_WIDTH);
        StringBuilder sb = new StringBuilder();
        for (int i = 0; i < row.length; i++) {
            sb.append(PadOnLeft(row[i], columnWidth[i]));
            if (i < row.length - 1) {
                sb.append(paddedSep);
            }
        }
    ...
    public static int[] MaxWidth( String[][] body, String[] header){
        ...
        int[] columnWidth = new int[numCols];

        UpdateMaxWidth(columnWidth, header);

        for (String[] row : body) {
            UpdateMaxWidth(columnWidth, row);
        }
    ...
    public static void PrintDashSeparator(int[] columnWidth){
        ...
        StringBuilder sb = new StringBuilder();
        String paddedDash = "-".repeat(SEP_WIDTH) + "+" + "-".repeat(SEP_WIDTH);
        for (int i = 0; i < columnWidth.length; i++) {
            sb.append("-".repeat(columnWidth[i]));
            if (i < columnWidth.length - 1) {
                sb.append(paddedDash);
            }
        }
        StdOut.println(sb.toString());
    }
